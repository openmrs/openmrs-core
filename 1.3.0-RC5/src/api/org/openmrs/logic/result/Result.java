...
public class Result extends ArrayList<Result> {
	...
	/**
	 * Returns the datatype of the result. If the result is a list of
	 * other results, then the datatype of the first element is returned
	 * 
	...
	public Boolean toBoolean() {
		...
		if (isSingleResult()) {
			...
			case NUMERIC:
				return (valueNumeric == null || valueNumeric == 0 ? false
				        : true);
			case TEXT:
				return (valueText == null || valueText.length() < 1 ? false
				        : true);
			default:
				return valueBoolean;
			}
	...
	/**
	 * @return the datetime representation of the result <em>value</em> (not
	 *         to be confused with the result's own datetime). For non-datetime
	 ...
	 *         representation of the first member is returned. <table>
	 *         <th>
	 *         <td>Datatype</td>
	 *         <td>Returns</td>
	 *         </th>
	...
	public Double toNumber() {
		if (isSingleResult()) {
			...
			case CODED:
				return 0D;
			case DATETIME:
				return (valueDatetime == null ? 0
				        : new Long(valueDatetime.getTime()).doubleValue());
			case NUMERIC:
	...
	public boolean containsConcept(Integer conceptId) {
		if (isSingleResult())
			return (valueCoded != null && valueCoded.getConceptId()
			                                        .equals(conceptId));
		for (Result r : this) {
...
//	}
	 ...
	 *         list, then this will return the result only if <em>index</em>
	 *         is equal to zero (0); otherwise, returns an empty result
	 * 
	...
	public Result latest() {
		...
		for (Result r : this) {
			if ((last.getResultDate() == null || r.getResultDate()
			                                          .after(last.getResultDate()))) {
				last = r;
			}
		}
		return last;
