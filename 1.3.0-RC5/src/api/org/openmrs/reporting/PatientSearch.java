...
/**
 * This class represents a search for a set of patients, as entered from a user
 * interface. There are different types of searches:
 * <ul> 
 ...
 *  <li>a regular search, which describes a PatientFilter subclass and a 
 *      list of bean-style properties to set.
 * <ul>
 * 
 * Composition filters: When isComposition() returns true, then this represents
 * something like "1 and (2 or 3)", which must be evaluated in the context of a
 * search history.
 * 
 * Saved filters: When isSavedFilterReference() returns true, then this
 * represents something like "saved filter #8" When isSavedCohortReference()
 * returns true, then this represents something like "saved cohort #3"
 * 
 * Regular filters: Otherwise this search describes a PatientFilter subclass and
 * a list of bean-style properties to set, so that it can be turned into a
 * PatientFilter with the utility method
 * OpenmrsUtil.toPatientFilter(PatientSearch). But it can also be left as-is for
 * better version-compatibility if PatientFilter classes change, or to avoid
...
public class PatientSearch implements CohortDefinition {
	...
	/**
	 * Convenience method so that a PatientSearch object can be created from
	...
	/**
	 * @return Whether this search requires a history against which to evaluate
	 *         it
	 */
	...
	/**
	 * Creates a copy of this PatientSearch that doesn't depend on history,
	 * replacing references with actual PatientSearch elements from the provided
	 * history. The PatientSearch object returned is only a copy when necessary
	 * to detach it from history. This method does NOT do a clone.
	 */
	public PatientSearch copyAndDetachFromHistory(CohortSearchHistory history) {
		if (isComposition() && requiresHistory()) {
			...
			copy.setParsedComposition(copyAndDetachHelper(parsedComposition,
			                                              history));
			return copy;
		} else
			return this;
	...
	private List<Object> copyAndDetachHelper(List<Object> list,
		...
		for (Object o : list) {
			if (o instanceof PatientSearch) {
				...
				PatientSearch ps = history.getSearchHistory()
				                          .get(((Integer) o) - 1);
	...
	/**
	 * Deep-copies this.parsedComposition, and converts to filters, in the
	 * context of history
	 */
	...
	private List<Object> cloneCompositionHelper(List<Object> list,
	        CohortSearchHistory history, EvaluationContext evalContext) {
		List<Object> ret = new ArrayList<Object>();
		for (Object o : list) {
			if (o instanceof List)
				...
				throw new RuntimeException("Programming Error: forgot to handle: "
				        + o.getClass());
		}
		return ret;
	}
	...
	public boolean isSavedReference() {
		return isSavedSearchReference() || isSavedFilterReference()
	...
	/**
	 * Call this to notify this composition search that the _i_th element of the
	 * search history has been removed, and the search potentially needs to
	 * renumber its constituent parts. Examples, assuming this search is "1 and
	 * (4 or 5)": * removeFromHistoryNotify(1) -> This search becomes "1 and (3
	 * or 4)" and the method return false * removeFromHistoryNotify(3) -> This
	 * search becomes invalid, and the method returns true *
	 * removeFromHistoryNotify(9) -> This search is unaffected, and the method
	 * returns false
	 * 
	 * @return whether or not this search itself should be removed (because it
	 *         directly references the removed history element
	 */
	...
	public void setFilterClass(Class clazz) {
		if (clazz != null && !PatientFilter.class.isAssignableFrom(clazz))
			throw new IllegalArgumentException(clazz
			        + " is not an org.openmrs.PatientFilter");
	...
	/**
	 * Adds a SearchArgument as a Parameter where the
	 * SearchArgument name is set to the Parameter label and
	 * SearchArgument value is set to the Parameter name and
	 * SearchArgument propertyClass is set to the Parameter clazz
	 * 
	 * @param parameter
	 */
	public void addParameter(Parameter parameter) {
		addArgument(parameter.getLabel(),
		            parameter.getName(),
		            parameter.getClazz());
	}
	...
	/**
	 * Elements in this list can be: an Integer, indicating a 1-based index into
	 * a search history a BooleanOperator (AND, OR, NOT) a PatientFilter a
	 * PatientSearch another List of the same form, which indicates a
