...
openmrs-mysql-createdb.sql		MySQL script to generate AMRS database
...
To install database:
	...
	1. If this is the first time you're installing the database OR
	   if you have dropped the AMRS database, run openmrs-mysql-createdb.sql

	2. Run openmrs-mysql.sql to create tables

	3. Run openmrs-mysql-data.sql to populate tables with data

	The createdb script simply creates an AMRS database for you.
