...
#----------------------------------------
# OpenMRS Datamodel version 1.5.0.03
# Brian McKown                 May 28, 2008
# Adding table concept_complex. 
# Adding column value_complex to obs table.
# Adding Concept Datatype "complex"
# Adding global_property obs.complex_obs_dir
# Deleted the mime_type and complex_obs tables
#----------------------------------------
...
BEGIN
    ...
    -- Add column to obs table.
    select 'Adding a value_complex column to the obs table.  This could take a long time.' AS '*** Note: ***' from dual;
    ALTER TABLE `obs` ADD COLUMN `value_complex` varchar(255)  DEFAULT NULL AFTER `value_text`;

    -- Create ConceptDatatype "Complex" if one does not already exist.
    IF (SELECT 1 FROM concept_datatype c where c.name='Complex') THEN
        SELECT 'A concept datatype named Complex already exists!' as 'WARNING:' from dual;
    ELSE
        INSERT INTO
          concept_datatype
          (name, hl7_abbreviation, description, creator, date_created) 
        VALUES (
          'Complex', 'ED', 'Complex value.  Analogous to HL7 Embedded Datatype', 1, NOW()
        );
    END IF;
    
    DROP TABLE `complex_obs`;
