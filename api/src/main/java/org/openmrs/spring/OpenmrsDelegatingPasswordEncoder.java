...
/**
 * A variation of Spring's {@code DelegatingPasswordEncoder} that falls back to the supplied
 * {@code fallbackEncoder} without using a prefix.
 * <p>
 * Spring's {@code DelegatingPasswordEncoder} winds up prepending {@code {<encoder>}} to the
 * password. However, we want to support existing passwords with no formatting, so with this
 * variation, our default fallback generates and validates passwords without that prefix.
 *
...
public class OpenmrsDelegatingPasswordEncoder implements PasswordEncoder {
	...
	public OpenmrsDelegatingPasswordEncoder(String idForEncode, Map<String, PasswordEncoder> idToPasswordEncoder,
	    PasswordEncoder fallbackEncoder) {
		if (idForEncode == null || idForEncode.isEmpty()) {
			...
			throw new IllegalArgumentException("The encoder named '" + idForEncode
			        + "' is not configured for this instance of OpenmrsDelegatingPasswordEncoder");
		} else {
