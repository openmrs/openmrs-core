...
public class JobRunrConfig {
	...
	@Bean
	public JobRunrConfiguration.JobRunrConfigurationResult jobRurnConfiguration(StorageProvider storageProvider, ApplicationContext applicationContext,
																			   @Value("${jobrunr.dashboard.username:admin}") String user,
																			   @Value("${jobrunr.dashboard.password:Admin123}") String password,
		...
		if (dashboardEnabled) {
			config.useDashboard(JobRunrDashboardWebServerConfiguration.usingStandardDashboardConfiguration()
				.andPort(port).andBasicAuthentication(user, password));
		}
