...
public class JobRunrSchedulerService extends BaseOpenmrsService implements SchedulerService {
		...
		return StreamSupport.stream(new Spliterators.AbstractSpliterator<TaskDetails>(Long.MAX_VALUE, Spliterator.ORDERED | Spliterator.NONNULL) {
			...
			public boolean tryAdvance(Consumer<? super TaskDetails> action) {
				if (currentBatch == null || !currentBatch.hasNext()) {
					int limit = 100;
					List<Job> jobs = storageProvider.getJobs(StateName.valueOf(state.name()), new OffsetBasedPageRequest("updatedAt:ASC", offset, limit)).getItems();
					if (jobs == null || jobs.isEmpty()) {
						return false;
					}
					// Filter tasks based on scheduled by or updated at
					currentBatch = jobs.stream().filter(job -> 
						(isSchedulerManager || isScheduledBy(job.getJobDetails(), userSystemId)) && 
							(before == null || job.getUpdatedAt().isBefore(before))).iterator();
					offset += jobs.size();
				}
	...
	public Stream<RecurringTaskDetails> getRecurringTasks() {
		...
		return storageProvider.getRecurringJobs().stream().filter(
			j -> isSchedulerManager || isScheduledBy(j.getJobDetails(), user.getSystemId()))
