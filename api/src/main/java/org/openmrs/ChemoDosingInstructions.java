...
public class ChemoDosingInstructions implements DosingInstructions {
	...
	public String getDosingInstructionsAsString(Locale locale) {
		...
		// serializing adjustment percentage, timing and dilution instructions to
		// compose dosing instructions
		if (this.dosageAdjustmentPercentage != null) {
			dosingInstructions.append(
					this.formatDosingInformation(ADJUSTMENT_PERCENTAGE, this.dosageAdjustmentPercentage.toString()));
		}
		...
		if (this.dosageDilutionInstructions != null) {
			dosingInstructions
					.append(this.formatDosingInformation(DILUTION_INSTRUCTIONS, this.dosageDilutionInstructions));
		}
	...
	public DosingInstructions getDosingInstructions(DrugOrder order) {
		...
		// dosing instructions in drug order consist of several lines each having a
		...
		while (tokenizeDosingInstructions.hasMoreTokens()) {
			...
			// Every instruction line is of the form "instructionLineType :
			// instructionLineValue"
			...
			if (instructionLineType.equals(ADJUSTMENT_PERCENTAGE)) {
				chemoDosingInstructions.dosageAdjustmentPercentage = Double.parseDouble(instructionLineValue);
			} else if (instructionLineType.equals(DELIVERED_TIMING)) {
