...
public class Context {
	 ...
	 * <br/>
	 * TODO Make thread-safe because this might be accessed by several thread at
	 * the same time. Making this thread safe might make this a bottleneck.
	 * 
	 * @param ctx
	 *            UserContext to set
	 */
	...
	public static UserContext getUserContext() {
		...
		if (log.isTraceEnabled())
			log.trace("Getting user context " + arr
	...
	public static void authenticate(String username, String password)
		...
		if (Daemon.isDaemonThread()) {
			log
					.error("Authentication attempted while operating on a "
	...
	private static javax.mail.Session getMailSession() {
		if (mailSession == null) {
			...
			Properties props = new Properties();
			props.setProperty("mail.transport.protocol", adminService
					.getGlobalProperty("mail.transport_protocol"));
			props.setProperty("mail.smtp.host", adminService
					.getGlobalProperty("mail.smtp_host"));
			props.setProperty("mail.smtp.port", adminService
					.getGlobalProperty("mail.smtp_port"));
			props.setProperty("mail.from", adminService
					.getGlobalProperty("mail.from"));
			props.setProperty("mail.debug", adminService
					.getGlobalProperty("mail.debug"));
			props.setProperty("mail.smtp.auth", adminService
					.getGlobalProperty("mail.smtp_auth"));
	 ...
	 * {@link DatabaseUpdater#update(Map)} will be required with a mapping from
	 ...
	 * If an {@link InputRequiredException} is thrown, a call to
	 * {@link DatabaseUpdater#update(Map)} will be required with a mapping from
	 * question prompt to user answer before startup can be called again. <br/>
	 * <br/>
	...
	public static void checkCoreDataset() {
		...
		try {
			Context
					.addProxyPrivilege(PrivilegeConstants.MANAGE_GLOBAL_PROPERTIES);
			Context
					.addProxyPrivilege(PrivilegeConstants.VIEW_GLOBAL_PROPERTIES);
			...
			Context
					.removeProxyPrivilege(PrivilegeConstants.MANAGE_GLOBAL_PROPERTIES);
			Context
					.removeProxyPrivilege(PrivilegeConstants.VIEW_GLOBAL_PROPERTIES);
		}
	}
	 ...
	 * 
	 * @param key
	 *            identifying string for this information
	 * @param value
