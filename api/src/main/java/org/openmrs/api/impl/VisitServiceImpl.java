...
public class VisitServiceImpl extends BaseOpenmrsService implements VisitService {
	...
	public void stopVisits(Date maximumStartDate) {
		...
		if (getVisitTypesToStop().isEmpty()) {
			return;
		}
		int counter = 0;
		Date stopDate = new Date();
		Visit nextVisit = dao.getNextVisit(null, getVisitTypesToStop(), maximumStartDate);
		...
		nextVisit = dao.getNextVisit(nextVisit, getVisitTypesToStop(), maximumStartDate);
		}
	}
