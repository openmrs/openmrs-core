...
public class HibernateDiagnosisDAO implements DiagnosisDAO {
	...
	private static final Log log = LogFactory.getLog(HibernateDiagnosisDAO.class);
...
//				+ "order by d.dateCreated desc");
			"from Diagnosis c where c.patient.patientId = :patientId and c.voided = false order "
				+ "by c.onsetDate desc");
	...
	public List<Diagnosis> getDiagnoses(Encounter encounter){
		Query query = sessionFactory.getCurrentSession().createQuery(
			"from Diagnosis d where d.encounter = :encounterId order by d.dateCreated desc");
