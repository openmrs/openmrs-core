...
public class Obs extends BaseFormRecordableOpenmrsData {
	 ...
	 * note: obsGroup.getConcept().isSet() should be true This will be non-null if
	 * this obs is a member of another groupedObs
	 * 
	 * @see #isObsGrouping() (??)
	 */
	 ...
	 * of: valueCoded, valueDrug, valueNumeric, or valueText
	 * 
	 ...
	 * in the list returned by <code>obsGroup</code>.{@link #getGroupMembers()}
	 * 
	 ...
	 * obsGroup.getGroupMembers(). That must be done (and should be done) manually.
	 * (I am not doing it here for fear of screwing up the normal loading and
	 * creation of this object via hibernate/spring)
	 * 
	 ...
	 * (either voided or non-voided) Note this method differs from
	 * hasGroupMembers(), as that method excludes voided obs; logic is that while a
	 * obs that has only voided group members should be seen as "having no group
	 ...
	 * ${obs.obsGroup} ? With this setup, ${obs.obsGrouping} returns a boolean of
	 * whether this obs is a parent and has members. ${obs.obsGroup} returns the
	 * parent object to this obs if this obs is a group member of some other group.
	 * 
	 ...
	 * has group members. By default, this ignores voided-objects. To include
	 * voided, use {@link #hasGroupMembers(boolean)} with value true.
	 * 
	 * @return true if this is the parent group of other obs <strong>Should</strong>
	 *         not include voided obs
	 */
	 ...
	 * obs has group members. The parameter specifies if this method whether or not
	 * voided obs should be considered.
	 * 
	 * @param includeVoided determines if Voided members should be considered as
	 *                      group members.
	 * @return true if this is the parent group of other Obs <strong>Should</strong>
	 *         return true if this obs has group members based on parameter
	 */
	 ...
	 * default this method only returns non-voided group members. To get all group
	 * members, use {@link #getGroupMembers(boolean)} with value true.
	 * <p>
	 * If it's not a group (i.e.
	 * {@link #getConcept()}.{@link org.openmrs.Concept#getSet()} is not true, then
	 * this returns null.
	 * 
	 ...
	 * will either return all group members, or only non-voided group members,
	 * depending on if the argument is set to be true or false respectively.
	 * 
	 * @param includeVoided
	 * @return the set of group members in this obs group <strong>Should</strong>
	 *         Get all group members if passed true, and non-voided if passed false
	 */
	 ...
	 * @see #addGroupMember(Obs)
	 * @see #hasGroupMembers() <strong>Should</strong> mark the obs as dirty when
	 *      the set is changed from null to a non empty one <strong>Should</strong>
	 *      not mark the obs as dirty when the set is changed from null to an empty
	 *      one <strong>Should</strong> mark the obs as dirty when the set is
	 *      replaced with another with different members <strong>Should</strong> not
	 *      mark the obs as dirty when the set is replaced with another with same
	 *      members
	 */
	 ...
	 * implicitly make this obs an ObsGroup.
	 * 
	 * @param member Obs to add to this group
	 * @see #setGroupMembers(Set)
	 * @see #getGroupMembers() <strong>Should</strong> return true when a new obs is
	 *      added as a member <strong>Should</strong> return false when a duplicate
	 *      obs is added as a member
	 */
	 ...
	 * link in the given <code>obs</code>object to this obs grouper
	 * 
	 * @param member Obs to remove from this group
	 * @see #setGroupMembers(Set)
	 * @see #getGroupMembers() <strong>Should</strong> return true when an obs is
	 *      removed <strong>Should</strong> return false when a non existent obs is
	 *      removed
	 */
	 ...
	 * ObsGroup: a Set&lt;Obs&gt; will be returned containing all of the children of
	 * this Obs' parent that are not ObsGroups themselves. This will include this
	 * Obs by default, unless getObsGroup() returns null, in which case an empty set
	 * is returned. If the Obs argument is an ObsGroup: a Set&lt;Obs&gt; will be
	 * returned containing 1. all of this Obs' group members, and 2. all ancestor
	 * Obs that are not themselves obsGroups.
	 * 
	 * @return Set&lt;Obs&gt;
	 */
	 ...
	 * but really the {@link #setPerson(Person)} method should be used like
	 ...
	 * 
	 * @return Boolean representation of the obs value <strong>Should</strong>
	 *         return true for value_numeric concepts if value is 1
	 *         <strong>Should</strong> return false for value_numeric concepts if
	 *         value is 0 <strong>Should</strong> return null for value_numeric
	 *         concepts if value is neither 1 nor 0
	 */
	 ...
	 * 
	 * @return true or false if value is set otherwise null <strong>Should</strong>
	 *         return true if value coded answer concept is true concept
	 ...
	 *         it includes a date component, that should be ignored.
	 * @since 1.9
	 */
	 ...
	 * @since 1.5 <strong>Should</strong> return true if the concept is complex
	 */
	 ...
	 * character: part A) the title; and part B) the URI. The title is the readable
	 * description of the valueComplex that is returned by
	 ...
	 * will persist the {@link ComplexData#getData()} object to the correct place
	 ...
	 * the file system or another location, not from the database. <br>
	 * <br>
	 * This will be null unless you call:
	 * 
	 * <pre>
	 * Obs obsWithComplexData = Context.getObsService().getComplexObs(obsId,
	 ...
	 * Obs is complex, returns the title of the complexData denoted by the section
	 * of getValueComplex() before the first bar '|' character; or returns the
	 * entire getValueComplex() if the bar '|' character is missing.
	 *
	 * @param locale locale for locale-specific depictions of value
	 *               <strong>Should</strong> return first part of valueComplex for
	 *               complex obs <strong>Should</strong> return first part of
	 *               valueComplex for non null valueComplexes
	 *               <strong>Should</strong> return non precise values for
	 *               NumericConcepts <strong>Should</strong> return date in correct
	 *               format <strong>Should</strong> not return long decimal numbers
	 *               as scientific notation <strong>Should</strong> use commas or
	 *               decimal places depending on locale <strong>Should</strong> not
	 *               use thousand separator <strong>Should</strong> return regular
	 *               number for size of zero to or greater than ten digits
	 ...
	 * question concept
	 *
	 * @param s the string to coerce to a boolean <strong>Should</strong> set value
	 *          as boolean if the datatype of the question concept is boolean
	 ...
	 * with the updates, and adds a reference to the previousVersion in the new Obs.
	 ...
	 * only time isDirty() is going to return false is when a new Obs has just been
	 * instantiated or loaded from the database and no method that modifies it
	 * internally has been invoked.
	 *
	 * @return true if not changed otherwise false
	 * @since 2.0 <strong>Should</strong> return false when no change has been made
	 *        <strong>Should</strong> return true when any immutable field has been
	 *        changed <strong>Should</strong> return false when only mutable fields
	 *        are changed <strong>Should</strong> return true when an immutable
	 *        field is changed from a null to a non null value
	 ...
	 * Observation Interpretation Codes. See
	 ...
	 * ObservationStatus values. At present OpenMRS does not support FHIR's
	 * REGISTERED and CANCELLED statuses, because we don't support obs with null
	 * values. See: https://www.hl7.org/fhir/valueset-observation-status.html
	 * 
	 * @since 2.1.0
	 */
