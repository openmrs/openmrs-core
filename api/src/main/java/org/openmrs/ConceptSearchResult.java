...
public class ConceptSearchResult implements java.io.Serializable {
	...
	/**
	 * Optional constructor for turning a conceptWord into a
	 * conceptSearchResult, the constructor is hidden from API users so as to
	 * hide the idea of conceptWord, it is meant to be used underneath the API
	 * for convenience purposes.
	 * 
	 * @param conceptWord
	 *            the conceptWord from which to construct a search result
	 */
	protected ConceptSearchResult(ConceptWord conceptWord) {
		if (conceptWord != null) {
			...
			if (conceptWord.getWeight() != null)
				this.weight = conceptWord.getWeight();
		}
	}
	 ...
	 */
	public ConceptSearchResult(String word, Concept concept,
			ConceptName conceptName, double weight) {
		this.concept = concept;
		this.conceptName = conceptName;
		this.word = word;
		this.weight = weight;
	...
	public boolean equals(Object obj) {
		if (obj instanceof ConceptSearchResult) {
			...
			return OpenmrsUtil.nullSafeEquals(this.getConcept(), result
					.getConcept())
					&& OpenmrsUtil.nullSafeEquals(this.getConceptName(), result
							.getConceptName())
					&& OpenmrsUtil.nullSafeEquals(this.getWord(), result
							.getWord())
					&& OpenmrsUtil.nullSafeEquals(this.getWeight(), result
							.getWeight());
		}
	...
	@Override
	public int hashCode() {
		if (this.getConcept() == null && this.getConceptName() == null
				&& this.getWord() != null && this.getWeight() == null)
			return super.hashCode();

		int hash = 5;
		if (this.getConcept() != null
				&& this.getConcept().getConceptId() != null)
			hash = 31 * getConcept().hashCode() + hash;
		if (this.getConceptName() != null
				&& this.getConceptName().getConceptNameId() != null)
			hash = 31 * getConceptName().hashCode() + hash;
		if (this.getWord() != null)
			hash = 31 * getWord().hashCode() + hash;
		if (this.getWeight() != null)
			hash = 31 * getWeight().hashCode() + hash;
