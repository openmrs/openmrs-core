...
public class TextHandler extends AbstractHandler implements ComplexObsHandler {
	...
	public Obs getObs(Obs obs, String view) {
		// Deprecated
		// Check if the file exists in the legacy location, if not use obs value complex as key
		String key = parseDataKey(obs);
		File file = AbstractHandler.getComplexDataFile(obs);
		boolean legacy = file.exists();
		if (legacy) {
		...
		if (ComplexObsHandler.TEXT_VIEW.equals(view) || ComplexObsHandler.RAW_VIEW.equals(view)) {
			...
			// Deprecated
			String title = legacy ? file.getName() : parseDataTitle(obs);
			String uri = legacy ? file.getPath() : key;
			// End of deprecated
			complexData = new ComplexData(title, uri);
		} else {
