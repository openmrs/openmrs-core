...
/**
 * Extend to aggregate events for specific event types that happen in a single transaction. Please
 * note that if transaction is not active, events are not aggregated.
 * <p>
 * It may be used to create a single event for a number of entity changes.
 * <p>
 * It is recommended for each aggregator to produce a custom type extending
 * {@link AggregatedEntityEvent}.
 * <p>
 * The event can be listened to with {@link org.springframework.context.event.EventListener},
 * {@link TransactionalEventListener} (<b>except for <i>TransactionPhase.BEFORE_COMMIT</i></b>) or
 * {@link org.openmrs.event.outbox.OutboxEventListener}.
 * <p>
 * Since this event is emitted in the BEFORE_COMMIT phase, any <i>@EventListener</i> for this event
 * is executed in the BEFORE_COMMIT phase as well.
 * <p>
 * <b>It is not supported to use <i>@TransactionalEventListener(phase =
	...
	private void registerTransactionSynchronization() {
		...
		TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
			...
			public void beforeCommit(boolean readOnly) {
				...
				@SuppressWarnings("unchecked")
				List<EntityEvent<?>> events = (List<EntityEvent<?>>) TransactionSynchronizationManager
				        .getResource(TransactionalEventAggregator.this);
