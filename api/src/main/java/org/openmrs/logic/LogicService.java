...
public interface LogicService {
	 ...
	 * <p>
	 * <strong>Should</strong> return all registered token matching the input fully<br/>
	 ...
	 * <p>
	 * <strong>Should</strong> not fail when another rule is registered on the same token<br/>
	 ...
	 * <p>
	 * <strong>Should</strong> not fail when no tags is specified<br/>
	 ...
	 * <p>
	 * <strong>Should</strong> return Rule associated with the input token<br/>
	...
	/**
	 * Evaluates multiple logic expressions for a single patient. (The expressions argument is an array
	 * and comes last because using a List would give this method the same type erasure as the
	...
	/**
	 * Evaluates multiple {@link LogicCriteria} for a single patient. (The criteria argument is an array
	 * and comes last because using a List would give this method the same type erasure as the
	 * {@link String}... version.)
	 *
	 * @param patientId which patient to run the rules on
	...
	/**
	 * Fetches the default datatype this token will return when fed to an eval() call. Results (returned
	 * by the logic service) are loosely typed by design; however, the default datatype can be a useful
	 * hint for managing user interfaces or providing default behavior when working with rules.
	 *
	 ...
	 * @return the logic data source with the given name or <code>null</code> if there is no data source
	 *         registered under the given name (must be an exact match)
	 */
