...
public class CreateDiscontinueOrders implements CustomTaskChange {
    ...
    private void createDiscontinueOrders(JdbcConnection connection, List<DiscontinuedOrder> discontinuedOrders) throws CustomChangeException, SQLException, DatabaseException {
        ...
        try {
            ...
            insertStatement = connection.prepareStatement("Insert into orders(previous_order_id, concept_id, patient_id, encounter_id, " +
                    "creator, date_created, date_stopped, discontinued_by, discontinued_reason, discontinued_reason_non_coded, uuid) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    private List<DiscontinuedOrder> getDiscontinuedOrders(JdbcConnection connection) throws CustomChangeException, SQLException {
        ...
        try {
            statement = connection.prepareStatement("select order_id, concept_id, patient_id, encounter_id, date_stopped, discontinued_by, discontinued_reason, discontinued_reason_non_coded from orders where discontinued = ?");
            statement.setBoolean(1, true);
            ResultSet rs = statement.executeQuery();
            while(rs.next()) {
                dc.add(new DiscontinuedOrder(
                        rs.getInt("order_id"),
    ...
    private static class DiscontinuedOrder {
        ...
        private DiscontinuedOrder(int orderId, int conceptId, int patientId, int encounterId, int discontinuedById, int discontinuedReasonId, String discontinuedReasonNonCoded, Date dateStopped) {
