...
public class OrderServiceTest extends BaseContextSensitiveTest {
	...
	/**
	 * @verifies pass if an active drug order for the same concept and care setting but different form exists
	 * @see OrderService#saveOrder(org.openmrs.Order, OrderContext)
	 */
	@Test
	public void saveOrder_shouldPassIfAnActiveOrderForTheSameConceptAndCareSettingButDifferentFormExists() throws Exception {
		executeDataSet("org/openmrs/api/include/OrderServiceTest-drugOrdersWitSameConceptAndDifferentFormAndStrength.xml");
		final Patient patient = patientService.getPatient(2);
		final Concept paracetamol = conceptService.getConcept(1000);
		//sanity check that we have an active order for the same concept
		DrugOrder existingOrder = (DrugOrder) orderService.getOrder(1000);
		assertTrue(existingOrder.isCurrent());
		assertEquals(paracetamol, existingOrder.getConcept());
		//New Drug order
		DrugOrder order = new DrugOrder();
		order.setPatient(patient);
		order.setCareSetting(orderService.getCareSetting(2));
		order.setConcept(paracetamol);
		order.setEncounter(encounterService.getEncounter(6));
		order.setOrderer(providerService.getProvider(1));
	...
	/**
	 * @verifies pass if an active drug order for the same concept and care setting but different strength exists
	 * @see OrderService#saveOrder(org.openmrs.Order, OrderContext)
	 */
	@Test
	public void saveOrder_shouldPassIfAnActiveOrderForTheSameConceptAndCareSettingButDifferentStrengthExists()
	        throws Exception {
		executeDataSet("org/openmrs/api/include/OrderServiceTest-drugOrdersWitSameConceptAndDifferentFormAndStrength.xml");
		final Patient patient = patientService.getPatient(2);
		final Concept paracetamol = conceptService.getConcept(1000);
		...
		order.setOrderer(providerService.getProvider(1));
		order.setCareSetting(existingOrder.getCareSetting());
		order.setDrug(conceptService.getDrug(3002));
		order.setDosingType(DrugOrder.DosingType.FREE_TEXT);
		order.setDosingInstructions("2 for 5 days");
		order.setQuantity(10.0);
		order.setQuantityUnits(conceptService.getConcept(51));
