...
public class PatientServiceTest extends BaseContextSensitiveTest {
	...
	public void getPatientIdentifiersByPatientProgram_shouldGetPatientsByIdentifierByProgram() throws Exception {
		...
		assertFalse("The patient List should not be null", panPatients.isEmpty());

		Patient patient = panPatients.iterator().next();

		Assert.assertEquals(patient.getActiveIdentifiers().size(), 2);
		...
		Assert.assertNotNull("Test Patient Program not found", patientProgram);

		List<PatientIdentifier> patientIdentifier = Context.getPatientService().getPatientIdentifiersByPatientProgram(patientProgram);

		Assert.assertEquals(patientIdentifier.size(), 1);
		Assert.assertEquals(patientIdentifier.iterator().next().getIdentifier(), "XXXCCCAAA11");
	}
