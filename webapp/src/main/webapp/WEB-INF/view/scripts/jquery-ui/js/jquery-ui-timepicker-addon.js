...
(function($) {
	...
	$.extend(Timepicker.prototype, {
		...
		_addTimePicker: function(dp_inst) {
			const currDT = (this.$altInput && this._defaults.altFieldTimeOnly) ?
		...
		_parseTime: function(timeString, withDate) {
			let regstr = this._defaults.timeFormat.toString()
		...
		_getFormatPositions: function() {
			const finds = this._defaults.timeFormat.toLowerCase().match(/(h{1,2}|m{1,2}|s{1,2}|l{1}|t{1,2}|z)/g),
		...
		_injectTimePicker: function() {
			...
			if ($dp.find("div#ui-timepicker-div-"+ dp_id).length === 0 && o.showTimepicker) {
				let noDisplay = ' style="display:none;"',
				...
				if (o.showHour && o.hourGrid > 0) {
					...
					for (let h = o.hourMin; h <= hourMax; h += parseInt(o.hourGrid,10)) {
						hourGridSize++;
						let tmph = (o.ampm && h > 12) ? h - 12 : h;
						if (tmph < 10) {
							tmph = '0' + tmph;
						}
						if (o.ampm) {
				...
				if (o.showMinute && o.minuteGrid > 0) {
					...
					for (let m = o.minuteMin; m <= minMax; m += parseInt(o.minuteGrid,10)) {
						minuteGridSize++;
						html += '<td>' + ((m < 10) ? '0' : '') + m + '</td>';
					}
				...
				if (o.showSecond && o.secondGrid > 0) {
					...
					for (s = o.secondMin; s <= secMax; s += parseInt(o.secondGrid,10)) {
						secondGridSize++;
						html += '<td>' + ((s < 10) ? '0' : '') + s + '</td>';
					}
				...
				if (o.showMillisec && o.millisecGrid > 0) {
					...
					html += '<dd class="ui_tpicker_millisec" id="ui_tpicker_millisec_' + dp_id + '"'	+
				...
				if (o.showHour && o.hourGrid > 0) {
					...
					$tp.find(".ui_tpicker_hour table").css({
						...
						$(this).click(function() {
							let h = $(this).html();
							if(o.ampm) {
								const ap = h.substring(2).toLowerCase(),
				...
				const onSelectDelegate = function () {
					tp_inst._onSelectHandler();
				};
		...
		_limitMinMaxDateTime: function(dp_inst, adjustSliders){
			...
			if(adjustSliders !== undefined && adjustSliders === true){
				const hourMax = (this._defaults.hourMax - ((this._defaults.hourMax - this._defaults.hourMin) % this._defaults.stepHour)).toFixed(0),
		...
		_onTimeChange: function() {
			let hour = (this.hour_slider) ? this.hour_slider.slider('value') : false,
			...
			const hasChanged = (hour !== this.hour || minute !== this.minute
		...
		_onSelectHandler: function() {
			const onSelect = this._defaults.onSelect;
		...
		_formatTime: function(time, format, ampm) {
			...
			let tmptime = (format || this._defaults.timeFormat).toString();
	...
	$.datepicker._doKeyPress = function(event) {
		...
		if (tp_inst) {
			if ($.datepicker._get(inst, 'constrainInput')) {
				const ampm = tp_inst._defaults.ampm,
...
//#######################################################################################
	...
	$.datepicker._gotoToday = function(id) {
		...
		if (tp_inst._defaults.showTimezone && tp_inst.timezone_select) {
			let tzoffset = now.getTimezoneOffset(); // If +0100, returns -60
...
//#######################################################################################
	$.datepicker._setTime = function(inst, date) {
		const tp_inst = this._get(inst, 'timepicker');
		if (tp_inst) {
			let defaults = tp_inst._defaults,
			...
			if ((hour < defaults.hourMin || hour > defaults.hourMax) || (minute < defaults.minuteMin ||
				minute > defaults.minuteMax) || (second < defaults.secondMin || second > defaults.secondMax) ||
...
//#######################################################################################
	...
	$.datepicker._formatDate = function(inst, day, month, year){
		...
		if(tp_inst) {
			if(day) {
				const b = this._base_formatDate(inst, day, month, year);
			}
