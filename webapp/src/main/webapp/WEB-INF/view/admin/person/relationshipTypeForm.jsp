...
<openmrs:require privilege="Manage Relationship Types"
	otherwise="/login.htm" redirect="/admin/person/relationshipType.form" />
...
<h2>
	<openmrs:message code="RelationshipType.title" />
</h2>
	...
	<fieldset>
		<table>
			<tr>
				...
				<td><spring:bind path="relationshipType.aIsToB">
			...
			<tr>
				...
				<td><spring:bind path="relationshipType.bIsToA">
						<input type="text" name="${status.expression}"
							value="${status.value}" size="35" />
						<c:if test="${status.errorMessage != ''}">
							<span class="error">${status.errorMessage}</span>
			...
			<tr>
				...
				<td valign="top"><spring:bind
						path="relationshipType.description">
			...
			<c:if test="${relationshipType.creator != null}">
				<tr>
					...
					<td>${relationshipType.creator.personName} - <openmrs:formatDate
							date="${relationshipType.dateCreated}" type="long" />
					</td>
				</tr>
			</c:if>
			<tr>
		...
		<br /> <input type="submit"
			value="<openmrs:message code="RelationshipType.save"/>" name="save">
	</fieldset>
</form>
...
<c:if
	test="${not relationshipType.retired && not empty relationshipType.relationshipTypeId}">
	<form method="post">
		<fieldset>
			<h4>
				<openmrs:message code="RelationshipType.retireRelationshipType" />
			</h4>
			...
			<br /> <input type="submit"
				value='<openmrs:message code="RelationshipType.retireRelationshipType"/>'
				name="retire" />
		</fieldset>
	</form>
</c:if>
...
<c:if
	test="${relationshipType.retired && not empty relationshipType.relationshipTypeId}">
	<form id="unretire" method="post">
		<fieldset>
			<h4>
				<openmrs:message code="RelationshipType.unretireRelationshipType" />
			</h4>
			<input type="submit"
...
<c:if test="${not empty relationshipType.relationshipTypeId}">
	<openmrs:hasPrivilege privilege="Purge Relationship Types">
		<form id="purge" method="post" onsubmit="return confirmPurge()">
			<fieldset>
				<h4>
					<openmrs:message code="RelationshipType.purgeRelationshipType" />
				</h4>
				<input type="submit"
					value='<openmrs:message code="RelationshipType.purgeRelationshipType"/>'
					name="purge" />
			</fieldset>
		</form>
