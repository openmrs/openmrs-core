<c:if
	test="${empty INCLUDE_PERSON_GENDER || (INCLUDE_PERSON_GENDER == 'true')}">
	<tr>
	...
	<tr>
		...
		<td><spring:bind path="gender">
				<openmrs:forEachRecord name="gender">
					<input type="radio" name="gender" id="${record.key}"
						value="${record.key}"
...
<tr>
	<td><openmrs:message code="Person.birthdate" /><br /> <i
		style="font-weight: normal; font-size: .8em;">(<openmrs:message
				code="general.format" />: <openmrs:datePattern />)
	</i></td>
	<td colspan="3"><script type="text/javascript">
		...
		function updateAge() {
			...
			try {
				var birthdate = parseSimpleDate(birthdateBox.value,
						'<openmrs:datePattern />');
				var age = getAge(birthdate);
				if (age > 0)
					ageBox.innerHTML = "(" + age
							+ ' <openmrs:message code="Person.age.years"/>)';
				else if (age == 1)
	...
	</script> <spring:bind path="birthdate">
			<input type="text" name="birthdate" size="10" id="birthdate"
				value="${status.value}"
		...
		</spring:bind> <span id="age"></span> &nbsp; <span id="birthdateEstimatedCheckbox"
		class="listItemChecked" style="padding: 5px;"> <spring:bind
				path="birthdateEstimated">
...
<openmrs:forEachDisplayAttributeType personType="" displayType="all"
	var="attrType">
				...
				<td><c:choose>
						<c:when test="${authorized == true}">
							<spring:bind path="attributeMap">
								<openmrs:fieldGen type="${attrType.format}"
...
<tr id="deathInformation">
	...
	<td style="white-space: nowrap"><spring:bind path="deathDate">
			<input type="text" name="deathDate" size="10" value="${status.value}"
				onfocus="showCalendar(this)" id="deathDate" />
			<i style="font-weight: normal; font-size: 0.8em;">(<openmrs:message
					code="general.format" />: <openmrs:datePattern />)
			</i>
			<c:if test="${status.errorMessage != ''}">
				<span class="error">${status.errorMessage}</span>
			</c:if>
		</spring:bind> &nbsp; &nbsp; <openmrs:message code="Person.deathdateEstimated" /> <spring:bind
			path="deathdateEstimated">
			<input type="hidden" name="_${status.expression}">
		...
		</spring:bind> &nbsp; <openmrs:message code="Person.causeOfDeath" /> <openmrs:globalProperty
			key="concept.causeOfDeath" var="conceptCauseOfDeath" /> <openmrs:globalProperty
			key="concept.otherNonCoded" var="conceptOther" /> <spring:bind
			path="causeOfDeath">
			<openmrs:fieldGen type="org.openmrs.Concept"
				formFieldName="causeOfDeath" val="${status.value}"
...
<spring:bind path="creator">
	<c:if test="${status.value != null}">
		<tr>
			...
			<td>${status.value.personName} - <openmrs:formatDate
					path="dateCreated" type="long" />
			</td>
		</tr>
	</c:if>
</spring:bind>
...
<spring:bind path="changedBy">
	<c:if test="${status.value != null}">
		<tr>
			...
			<td colspan="2">${status.value.personName} - <openmrs:formatDate
					path="dateChanged" type="long" />
			</td>
		</tr>
	</c:if>
</spring:bind>
