...
<openmrs:require privilege="Edit Forms" otherwise="/login.htm"
	redirect="/admin/forms/formEdit.form" />

<%@ include file="/WEB-INF/template/header.jsp"%>
<%@ include file="localHeader.jsp"%>

<h2>
	<openmrs:message code="Form.edit.title" />
</h2>
...
<c:if test="${form.retired}">
	<div class="retiredMessage">
		<div>
			<openmrs:message code="Form.retiredMessage" />
		</div>
	</div>
</c:if>
...
|
<a href="formResources.form?formId=${ form.formId }"><openmrs:message
			code="Form.manageResources" /></a>
	<c:if test="${not isBasicForm}">
		<openmrs:extensionPoint pointId="org.openmrs.admin.forms.formHeader"
			type="html" parameters="formId=${form.formId}">
			<c:forEach items="${extension.links}" var="link">
			| <a href="<c:url value="${link.key}" />"><openmrs:message
						code="${link.value}" /></a>
			</c:forEach>
...
<form method="post" enctype="multipart/form-data">
	<table>
		...
		<tr>
			...
			<td valign="top"><spring:bind path="form.description">
		...
		<tr>
			...
			<td><spring:bind path="form.version">
					<input type="text" name="${status.expression}"
		...
		<tr>
			...
			<td><spring:bind path="form.published">
		...
		<tr>
			...
			<td><spring:bind path="form.encounterType">
		...
		<tr>
			...
			<td><spring:bind path="form.retired">
					<input type="hidden" name="_${status.expression}">
					<input type="checkbox" name="${status.expression}"
						id="${status.expression}"
		...
		<tr id="retiredReasonRow">
			...
			<spring:bind path="form.retireReason">
				<td><input type="text" name="${status.expression}"
					id="retiredReason" value="${status.value}" /> <c:if
		...
		<c:if test="${form.retired}">
			<tr>
				...
				<td>${form.retiredBy.personName} - <openmrs:formatDate
						date="${form.dateRetired}" type="long" />
				</td>
			</tr>
		</c:if>
		<c:if test="${!(form.creator == null)}">
			<tr>
				...
				<td>${form.creator.personName} - <openmrs:formatDate
						date="${form.dateCreated}" type="long" />
				</td>
			</tr>
		</c:if>
		<c:if test="${!(form.changedBy == null)}">
			<tr>
				...
				<td>${form.changedBy.personName} - <openmrs:formatDate
						date="${form.dateChanged}" type="long" />
				</td>
			</tr>
		</c:if>
		<openmrs:extensionPoint pointId="org.openmrs.admin.forms.formRow"
			type="html" parameters="formId=${form.formId}"
	...
	<c:if test="${not empty param.duplicate}">
		<input type="submit" name="action"
			value="<openmrs:message code="Form.create.duplicate"/>">
	</c:if>
	<c:if test="${empty param.duplicate}">
		<input type="submit" name="action"
			value="<openmrs:message code="Form.save"/>">

		<c:if test="${form.formId != null && not isBasicForm}">
			<openmrs:hasPrivilege privilege="Delete Forms">
			 &nbsp; &nbsp; &nbsp;
			<input type="submit" name="action"
					value="<openmrs:message code="Form.delete"/>"
...
<script type="text/javascript">
	document.getElementById('retiredReasonRow').style.display = document
			.getElementById('retired').checked == true ? '' : 'none';
</script>
...
a.delete {
	background: url(${pageContext.request.contextPath}/images/delete.gif)
...
#fieldSearchDiv {
	...
	<c:if test="${form.published == true}">
		<div class="retiredMessage">
			<div>
				<openmrs:message code="Form.design.disabled" />
			</div>
		</div>
	</c:if>

	<div dojoType="TreeBasicController" widgetId="treeController"
	...
	<div dojoType="TreeContextMenu" toggle="explode"
		...
		<div dojoType="TreeMenuItem" treeActions="edit" caption="Edit Field"
			widgetId="treeContextMenuEdit"></div>
		<div dojoType="TreeMenuItem" treeActions="addChild" caption="Create"
			widgetId="treeContextMenuCreate"></div>
		<div dojoType="TreeMenuItem" treeActions="remove" caption="Remove"
			widgetId="treeContextMenuRemove"></div>
	</div>
	...
	<table width="99%">
		<tr>
			<td valign="top">
				<div dojoType="Tree" menu="treeContextMenu" strictFolders="false"
					DNDMode="between" toggler="fade" widgetId="tree"
					DNDAcceptTypes="<c:if test="${form.published != true}">*</c:if>"
					controller="treeController" selector="treeSelector"></div>
			</td>
			<td valign="top" style="padding-left: 5px;" id="fieldSearch"
				width="40%">
				<div id="fieldSearchDiv">
					<c:if test="${form.published != true && not isBasicForm}">
						<div dojoType="FieldSearch" widgetId="fieldSearch"
							searchLabel='<openmrs:message code="Field.find" />'
							showHeaderRow="false" alsoSearchConcepts="true"></div>
					</c:if>
				</div>
			</td>
		</tr>
	...
	<div id="editFormField">
		<div id="formFieldTitle">
			<openmrs:message code="FormField.edit" />
			:
		</div>
		...
		<form xonsubmit="save(selectedNode)"
			...
			<c:if test="${form.published != true && not isBasicForm}">
				<input type="submit" id="saveFormField"
					onclick="return save(selectedNode);"
