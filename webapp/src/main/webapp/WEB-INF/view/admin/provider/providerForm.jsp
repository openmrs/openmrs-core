...
<openmrs:require privilege="View Providers" otherwise="/login.htm"
	redirect="/admin/provider/provider.form" />
...
<h2>
	<openmrs:message code="Provider.manage.title" />
</h2>
...
<b class="boxHeader"> <c:if test="${provider.providerId == null}">
		<openmrs:message code="Provider.create" />
	</c:if> <c:if test="${provider.providerId != null}">
	...
	<form method="post">
		...
		<table cellpadding="3" cellspacing="0">
			...
			<tr>
				...
				<td colspan="4"><spring:bind path="provider.identifier">
			...
			<c:choose>
				<c:when test="${provider.providerId == null}">
					<tr>
						...
						<td><spring:bind path="provider.person">
								<openmrs:fieldGen type="org.openmrs.Person"
						...
						<td>&nbsp;&nbsp;&nbsp;<openmrs:message code="general.or" />&nbsp;&nbsp;&nbsp;
						</td>
						<th><openmrs:message code="Provider.name" /></th>
						<td><spring:bind path="provider.name">
					...
					<tr>
						...
						<td>
							<div class="providerDetails"
								<c:if test="${provider.person != null}">style="display:none"</c:if>>
								<form:input id="providerName" path="provider.name" />
								<form:errors path="provider.name" cssClass="error" />
								<openmrs:message code="general.or" />
								<a href="javascript:void(0)" onclick="toggleProviderDetails()">
									<openmrs:message code="Provider.linkToPerson" />
								</a>
							</div>
							<div class="providerDetails"
								<c:if test="${provider.person == null}">style="display:none"</c:if>>
								${provider.person.personName} <span
									<c:if test="${provider.person != null}">style="display:none"</c:if>>
								...
								</span> <a href="javascript:void(0)" onclick="toggleProviderDetails()">(<openmrs:message
										code="Provider.unLinkFromPerson" />)
								</a>
							</div> <input id="linkToPerson" name="linkToPerson" type="checkbox"
			...
			<spring:bind path="provider.activeAttributes">
				<c:if test="${status.error}">
					<tr>
						<th></th>
						<td><span class="error"> <c:forEach var="err"
			...
			<c:if test="${ not empty providerAttributeTypes }">
				<c:forEach var="attrType" items="${ providerAttributeTypes }">
					<openmrs_tag:attributesForType attributeType="${ attrType }"
		...
		<br /> <input type="hidden" name="phrase"
			...
			name="saveProviderButton"
			value='<openmrs:message code="Provider.save"/>'> &nbsp; <input
			type="button" value='<openmrs:message code="general.cancel"/>'
			onclick="document.location='index.htm'">
		...
		<form method="post">
			<table cellpadding="3" cellspacing="0">
				<tr>
					...
					<td><a href="#View User"
						onclick="return gotoUser(null, '${provider.creator.userId}')">${provider.creator.personName}</a>
				...
				<c:if test="${provider.retiredBy == null}">
					<tr id="retiredReason">
						...
						<td><spring:bind path="provider.retired">
								<input type="hidden" name="${status.expression}" value="true" />
							</spring:bind> <spring:bind path="provider.retireReason">
								<input type="text" id="retire" value="${status.value}"
				...
				<c:if test="${provider.retiredBy != null}">
					<tr id="retiredBy">
						...
						<td><a href="#View User"
							onclick="return gotoUser(null, '${provider.retiredBy.userId}')">${provider.retiredBy.personName}</a>
							- <openmrs:formatDate date="${provider.dateRetired}"
								type="medium" /></td>
					</tr>
					<tr>
						<th><openmrs:message code="general.retiredReason" /></th>
