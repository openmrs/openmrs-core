...
<openmrs:require privilege="Manage Locations" otherwise="/login.htm"
	redirect="/admin/locations/location.form" />

<%@ include file="/WEB-INF/template/header.jsp"%>
<%@ include file="localHeader.jsp"%>

<h2>
	<openmrs:message code="Location.title" />
</h2>

<openmrs:extensionPoint
	pointId="org.openmrs.admin.locations.locationForm.afterTitle"
	type="html" parameters="locationId=${location.locationId}" />
	...
	<form action="" method="post">
		<div class="retiredMessage">
			<div>
				...
				- ${location.retireReason} <input type="submit"
					value='<openmrs:message code="Location.unretireLocation"/>'
					name="unretireLocation" />
			</div>
		</div>
	</form>
</c:if>
...
<spring:hasBindErrors name="location">
	...
	<div class="error">
		<c:forEach items="${errors.globalErrors}" var="error">
			<openmrs:message code="${error.defaultMessage}"
	...
	<fieldset>
		<table class="left-aligned-th">
			<tr>
				...
				<td colspan="5"><spring:bind path="location.name">
			...
			<tr>
				...
				<td valign="top" colspan="5"><spring:bind
			...
			<spring:nestedPath path="location">
				<openmrs:portlet url="addressLayout" id="addressPortlet" size="full"
					parameters="layoutShowTable=false|layoutShowExtended=false|layoutShowErrors=false" />
			</spring:nestedPath>
			<tr>
				<th valign="top"><openmrs:message
						code="Location.parentLocation" /></th>
				<td colspan="5"><spring:bind path="location.parentLocation">
						<openmrs_tag:locationField formFieldName="parentLocation"
							initialValue="${status.value}" optionHeader="[blank]" />
						<c:if test="${status.errorMessage != ''}">
							<span class="error">${status.errorMessage}</span>
						</c:if>
					</spring:bind></td>
			...
			<spring:bind path="location.activeAttributes">
				<c:if test="${status.error}">
					<tr>
						<th></th>
						<td colspan="5"><span class="error"> <c:forEach
									var="err" items="${status.errorMessages}">
								${ err }<br />
								</c:forEach>
						</span></td>
					</tr>
			...
			<tr>
				...
				<td colspan="5"><spring:bind path="location.tags">
			...
			<c:if test="${!(location.creator == null)}">
				<tr>
					...
					<td colspan="5">${location.creator.personName} - <openmrs:formatDate
							date="${location.dateCreated}" type="long" />
					</td>
				</tr>
			</c:if>
			<tr>
		...
		<openmrs:extensionPoint
			pointId="org.openmrs.admin.locations.locationForm.inForm" type="html"
			parameters="locationId=${location.locationId}" />
		<br /> <input type="submit"
			value="<openmrs:message code="Location.save"/>" name="saveLocation">
	</fieldset>
</form>
...
<c:if test="${not location.retired && not empty location.locationId}">
	<form action="" method="post">
		<fieldset>
			<h4>
				<openmrs:message code="Location.retireLocation" />
			</h4>
			...
			<input type="hidden" name="retired" value="true" /> <br /> <input
				type="submit"
				value='<openmrs:message code="Location.retireLocation"/>'
				name="retireLocation" />
		</fieldset>
	</form>
</c:if>
