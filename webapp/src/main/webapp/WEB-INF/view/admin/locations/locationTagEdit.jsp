...
<openmrs:require privilege="Manage Location Tags" otherwise="/login.htm"
	redirect="/admin/locations/locationTag.list" />

<%@ include file="/WEB-INF/template/header.jsp"%>
<%@ include file="localHeader.jsp"%>

<h2>
	<openmrs:message code="LocationTag.edit.title" />
</h2>
	...
	<form method="post" action="locationTagUnretire.form">
		...
		<div class="retiredMessage">
			<div>
				...
				- ${locationTag.retireReason} <input type="submit"
...
<div class="box">
	<form:form modelAttribute="locationTag">
		<table>
			<tr>
				<td><span class="required">*</span> <openmrs:message
						code="LocationTag.name" /></td>
				<td><form:input path="name" /> <form:errors path="name"
						cssClass="error" /></td>
			</tr>
			<tr>
				<td><openmrs:message code="LocationTag.description" /></td>
				<td><form:textarea path="description" rows="3" cols="72" /> <form:errors
						path="description" cssClass="error" /></td>
			</tr>
			<tr>
				<td><openmrs:message code="general.createdBy" /></td>
				<td><openmrs:format user="${locationTag.creator}" /> <openmrs:formatDate
						date="${locationTag.dateCreated}" /></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><input type="submit"
					value="<openmrs:message code="general.save"/>" /> <input
					type="button" value="<openmrs:message code="general.cancel"/>"
					onClick="window.location = 'locationTag.list'" /></td>
			</tr>
		</table>
	</form:form>
</div>
...
<div class="box">
	...
	<c:if test="${not locationTag.retired && not empty locationTag.id}">
		...
		<form method="post" action="locationTagRetire.form">
			<input type="hidden" name="id" value="${locationTag.id}" /> <b><openmrs:message
					code="general.retire" /></b> <br />
			<openmrs:message code="general.reason" />
			: <input type="text" name="retireReason" size="40" /> <input
