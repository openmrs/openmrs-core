...
<openmrs:require privilege="Manage Privileges" otherwise="/login.htm"
	redirect="/admin/users/privilege.form" />

<%@ include file="/WEB-INF/template/header.jsp"%>
<%@ include file="localHeader.jsp"%>

<h2>
	<openmrs:message code="Privilege.manage.title" />
</h2>
...
<form method="post">
	<table>
		<tr>
			...
			<td><spring:bind path="privilege.privilege">
					<c:if test="${status.value == null || status.value == \"\"}">
						<input type="text" name="${status.expression}" id="priv"
							value="${status.value}">
					</c:if>
		...
		<tr>
			...
			<td valign="top"><spring:bind path="privilege.description">
