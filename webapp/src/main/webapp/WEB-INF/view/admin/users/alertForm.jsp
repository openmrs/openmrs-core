...
<openmrs:require privilege="Manage Alerts" otherwise="/login.htm"
	redirect="/admin/users/alert.form" />
...
<script type="text/javascript">
	...
	dojo.addOnLoad(function() {
		...
		dojo.event.topic.subscribe("uSearch/select", function(msg) {
...
<h2>
	<openmrs:message code="Alert.manage.title" />
</h2>
...
<form method="post">
	<table>
		<tr>
			...
			<td><spring:bind path="alert.text">
		...
		<tr>
			...
			<td valign="top"><input type="hidden" name="userIds"
				id="userIds" size="40"
				value='<c:forEach items="${alert.recipients}" var="recipient">${recipient.recipient.userId} </c:forEach>' />
				<table cellpadding="0" cellspacing="0">
					<tr>
						<td valign="top"><select class="mediumWidth" size="6"
							id="userNames" multiple
						...
						<td valign="top" class="buttons">&nbsp;<span
							dojoType="UserSearch" widgetId="uSearch"></span><span
							dojoType="OpenmrsPopup" searchWidget="uSearch"
							...
							&nbsp; <input type="button"
							value="<openmrs:message code="User.goTo"/>" class="smallButton"
		...
		<tr>
			...
			<td valign="top"><select id="roleStr" class="mediumWidth">
					<option value="">
						<openmrs:message code="general.none" />
					</option>
					<c:forEach items="${allRoles}" var="role">
						<option value="${role.role}"
							<c:if test="${role == status.value}">selected</c:if>>${role.role}</option>
					</c:forEach>
			</select> &nbsp;<input type="button" class="smallButton"
				value="<openmrs:message code="Alert.addRole"/>" onClick="addRole();" />
				<input type="hidden" name="newRoles" id="newRoles" value="" /></td>
		</tr>
		<tr>
			<th></th>
			<td>
				<table cellpadding="0" cellspacing="0">
					<tr>
						<td><select class="mediumWidth" size="3" multiple id="roles"
							onkeydown="listKeyPress('roles', 'newRoles', ',', event);">
						</select></td>
						<td valign="top" class="buttons">&nbsp; <input type="button"
							value="<openmrs:message code="general.remove"/>"
							class="smallButton"
		...
		<tr>
			...
			<td valign="top"><spring:bind path="alert.satisfiedByAny">
					<input type="hidden" name="_${status.expression}" value="on" />
					<input type="checkbox" name="${status.expression}" value="on"
						<c:if test="${alert.satisfiedByAny}">checked</c:if> />
				</spring:bind> <i><openmrs:message code="Alert.satisfiedByAny.description" /></i>
			</td>
		</tr>
		<tr>
			<th valign="top"><openmrs:message code="Alert.dateToExpire" /></th>
			<td valign="top"><spring:bind path="alert.dateToExpire">
		...
		<c:if test="${!(encounter.changedBy == null)}">
			<tr>
				...
				<td><a href="#View User"
					onclick="return gotoUser(null, '${alert.changedBy.userId}')">${alert.changedBy.personName}</a>
