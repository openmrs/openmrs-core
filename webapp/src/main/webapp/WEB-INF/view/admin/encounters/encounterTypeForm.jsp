	...
	<tr>
		...
		<td valign="top">
			<spring:bind path="encounterType.description">
				<textarea name="description" rows="3" cols="40" onkeydown="return forceMaxLength(this, 1024);" >${status.value}</textarea>
				<c:if test="${status.errorMessage != ''}"><c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if></c:if>
			</spring:bind>
			<span id="show_error" class="error" style=display:none><openmrs:message code="error.exceededMaxLengthOfField" arguments="1024"/></span>
		</td>
	</tr>
	<tr>
