	...
	<tr>
		...
		<td valign="top">
			<spring:bind path="encounterType.description">
				...
				onfocusout="return forceMaxLength(this, 'errorFieldId', 1024);" 
				onkeypress="return forceMaxLength(this, 'errorFieldId', 1024);" 
				onkeyup="return forceMaxLength(this, 'errorFieldId', 1024);" >${status.value}</textarea>
				<c:if test="${status.errorMessage != ''}"><c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if></c:if>
			</spring:bind>
