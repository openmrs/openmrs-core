					...
					<tr class="patientRegimenRow">
						...
						<td valign="top" align="right" class="patientRegimeDataFlexible">
							<div id="regimenPortletAddFlexible">
								<form method="post" id="orderForm" onSubmit="handleAddDrugOrder(<c:out value="${model.patientId}" />, 'drug', 'dose', 'units', 'frequencyDay', 'frequencyWeek', 'startDate')">
	...
	<script>
		...
		<c:if test="${not empty model.standardRegimens}">
			...
			function addStandardDrug(drugId, brandName, dose, units, frequency, instructions, startDate) {
				DWROrderService.createDrugOrder(<c:out value="${model.patientId}" />, drugId, dose, units, frequency, startDate, instructions, dwrOrderNotifyComplete);
			}
		...
		function addNewComponent() {
			handleAddDrugOrder(<c:out value="${model.patientId}" />, 'drug', 'dose', 'units', 'frequencyDay', 'frequencyWeek', 'startDate');
		}
