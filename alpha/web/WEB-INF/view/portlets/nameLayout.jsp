...
<c:if test="${model.authenticatedUser != null}">
	<c:choose>
						...
						<c:if test="${model.layoutShowExtended == 'true'}">
							<td>
								<spring:bind path="preferred">
									...
									<input type="checkbox" name="${status.expression}" onclick="preferredBoxClick(this)" alt="personName" <c:if test="${status.value == true}">checked</c:if> />
					...
					<c:if test="${model.layoutShowExtended == 'true'}">
							<tr>
								...
								<td>
									<spring:bind path="preferred">
										<input type="hidden" name="_${status.expression}">
										<input type="checkbox" name="${status.expression}" onclick="if (preferredBoxClick) preferredBoxClick(this)" alt="personName" <c:if test="${status.value == true}">checked</c:if> />
										<c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
									</spring:bind>
								</td>
							</tr>
		...
		<c:otherwise>
			ERROR! unknown size '${model.size}' in Name portlet
		</c:otherwise>
	</c:choose>
</c:if>
