...
public class PatientSetController implements Controller {
	 ...
	 */
	public ModelAndView setPatientSet(HttpServletRequest request, HttpServletResponse response) throws ServletException,
	                                                                                           IOException {
	 ...
	 */
	public ModelAndView clearPatientSet(HttpServletRequest request, HttpServletResponse response) throws ServletException,
	                                                                                             IOException {
	...
	/**
	 * Adds to the PatientSet in the user's session. Adds a single patientId from the "patientId"
	 * parameter, or a comma-separated list from the "patientIds" parameter.
	 */
	public ModelAndView addToSet(HttpServletRequest request, HttpServletResponse response) throws ServletException,
	                                                                                      IOException {
		...
		if (patientSet.size() > 0) {
			if ("true".equals(request.getParameter("appendPatientId")))
				url += (url.indexOf('?') >= 0 ? "&" : "?") + "patientId="
				        + (id != null ? id : patientSet.getMemberIds().iterator().next());
			if ("true".equals(request.getParameter("showPatientSet")))
				url += (url.indexOf('?') >= 0 ? "&" : "?") + "showPatientSet=true";
		}
	...
	/**
	 * Removes patients from the PatientSet in the user's session. Removes a single patientId from
	 * the "patientId" parameter, or a comma-separated list from the "patientIds" parameter.
	 */
	public ModelAndView removeFromSet(HttpServletRequest request, HttpServletResponse response) throws ServletException,
	                                                                                           IOException {
