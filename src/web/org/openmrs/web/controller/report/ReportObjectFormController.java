	...
	protected void onBind(HttpServletRequest request, Object obj, BindException be) throws Exception {
		...
		if (correspondingValidatorName.length() > 0) {
			try {
				...
				log.error("Could not instantiate validator \"" + correspondingValidatorName + "\" for ReportObject class "
				        + currentClassName);
			}
		} else {
			try {
				...
				log.debug("Could not instantiate default validator \"" + currentClassName
				        + "Validator\" for ReportObject class " + currentClassName + "; Using default validator: "
				        + rs.getDefaultReportObjectValidator());
		...
		for (Field field : obj.getClass().getDeclaredFields()) {
			...
			if ((modifiers & (0x0 | Modifier.TRANSIENT)) == (0x0 | Modifier.TRANSIENT)) {
				...
				Method m = obj.getClass().getMethod(
				    "get" + fieldName.substring(0, 1).toUpperCase() + fieldName.substring(1), (Class[]) null);
