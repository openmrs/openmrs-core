...
/**
 * This is the java controller for the /openmrs/admin/encounters/encounterDisplay.list
 * page. The jsp for this display popup is located at 
 * /web/WEB-INF/view/encounters/encounerDisplay.jsp
 */
public class EncounterDisplayController implements Controller {
	...
	/**
	 * The page that obs are put on if they are not given a page number
	 * in their associated FormField object
	 */
	...
	/**
     * This will look through all form fields and find the one that has
     * a concept that matches the given concept
     * 
     * If one is found, that formfield is removed from the given list
     * <code>formFields</code>
     * 
     * If there are none found, null is returned.
     * 
     ...
     */
    private FormField popFormFieldForConcept(List<FormField> formFields,
            Concept conceptToSearchFor) {
	...
	public class FieldHolder implements Comparable<FieldHolder> {
		...
		/**
		 * The formfield that represents the labeling of this row
		 * This is also used in the sorting. See {@link #compareTo(org.openmrs.web.controller.encounter.EncounterDisplayController.FieldHolder)}
		 * See {@link #getLabel()} for the labeling
		 */
		private FormField formField = null;
		
		/**
		 * these are the rows in the obsGroup table
		 * If this is not an obs grouper, this will contain
		 * only one obs
		 */
		...
		/**
		 * A row must be created with both a FormField to act
		 * as its label and an obs that is the first of possibly
		 * several rows to display 
		 * 
		...
		public FieldHolder(FormField formField, Obs obsToAdd) throws Exception {
			...
			if (obsToAdd.getObsGroup() != null) {
				throw new Exception("FieldHolders only contain top-level obs.  This obs is contained in some other group, it is added automagically there. " + obsToAdd);
			}
		}
		
		/**
		 * public getter method for the formfield that is the 
		 * label for this row
		 * 
		...
		/**
		 * public getter for the columns (that are unique concepts
		 * across all obs in this FieldHolder)
		 * 
		...
		/**
		 * public getter for the obs that are the different
		 * rows for this FieldHolder.  If this isn't a grouping
		 * type of row, the set could still have multiple obs in it
		 * because there are multiple questions (obs) in this encounter
		 * that are asking the same thing (same concept)
		 * 
		...
		/**
		 * Convenience method to know whether this row is an obs grouping
		 * and should be displayed with a table or if its a single
		 * one-and-done obs and should just be shown as one value
		 * 
		...
		/**
		 * List of columns for each obs grouper in this fieldholder. Not 
		 * every grouper will have every column (concept), so some cells
		 * will be null.  The columns are determined by getObsGroupConcepts()
		 * 
		...
		/**
		 * Add another obs grouper to this row
		 * 
		 * This method shouldn't be called with obs that are 
		 * within another grouped obs.  This should only be called
		 * with the parent obs grouper.
		 * 
		...
		public int compareTo(FieldHolder other) {
			int temp = OpenmrsUtil.compareWithNullAsGreatest(formField.getPageNumber(), other.getFormField().getPageNumber());
			if (temp == 0)
				temp = OpenmrsUtil.compareWithNullAsGreatest(formField.getFieldNumber(), other.getFormField().getFieldNumber());
			if (temp == 0)
			...
			if (temp == 0 && formField.getPageNumber() == null && formField.getFieldNumber() == null && formField.getFieldPart() == null)
				temp = OpenmrsUtil.compareWithNullAsGreatest(formField.getSortWeight(), other.getFormField().getSortWeight());
			return temp;
		}
		 ...
		 * 
		 * This is produced from the formfield associated with this row
		 * 
		...
		/**
		 * Convenience method to get the page number for this row.  This just
		 * checks the associated form field for its assigned page number.  If the
		 * formfield doesn't have a page, this row is thrown on the DEFAULT_PAGE_NUMBERth page.
		 * 
