...
public class XsnUploadFormController extends SimpleFormController {
	...
	protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object obj, BindException errors) throws Exception {
		...
		if (context != null && context.isAuthenticated()) {
			...
			String xsnForm = request.getParameter("xsnForm");
			if (xsnForm != null && xsnForm.length() > 0) {
			
				Integer formId = Integer.valueOf(xsnForm);
				...
				try {
					...
					if (request instanceof MultipartHttpServletRequest) {
						...
						if (xsnFile != null && !xsnFile.isEmpty()) {
							PublishInfoPath.publishXSN(xsnFile.getInputStream(), form);
