...
public class HibernateLogicEncounterDAO implements LogicEncounterDAO {
	...
	private Criterion getCriterion(LogicExpression logicExpression, Date indexDate) {
		...
		if (operator == null){
			...
			if (rightOperand instanceof Date)
				criterion.add(Restrictions.eq("encounterDatetime",
					                    rightOperand));
			else
			...
			if (rightOperand instanceof Date)
				criterion.add(Restrictions.le("encounterDatetime",
					                    rightOperand));
			else
			...
			if (rightOperand instanceof Date)
				criterion.add(Restrictions.ge("encounterDatetime",
					                    rightOperand));
			else
			...
			if (rightOperand instanceof Date)
				criterion.add(Restrictions.lt("encounterDatetime",
					                    rightOperand));
			else
			...
			if (rightOperand instanceof Date)
				criterion.add(Restrictions.gt("encounterDatetime",
					                    rightOperand));
			else
	...
	private List<Encounter> logicToHibernate(LogicExpression expression,Cohort who) {
		...
		if (transformOperator == Operator.LAST) {
			...
			addOrder(Order.asc("encounterId"));
