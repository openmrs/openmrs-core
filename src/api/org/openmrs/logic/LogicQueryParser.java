...
/**
 * Parses strings into a LogicCriteria object. This provides a convenience
 * mechanism for logic queries to be written out in a simplified, text-based
 * query language rather than being built manually in Java. It also provides a
 * simpler mechanism for external code to send queries to the logic service.
 * 
 * A simple logic query follows the form:
 * 
 * <code>[ <aggregator> ] {TOKEN} [ <conditions> ]</code>
 * 
 * Both the aggregator and the conditions are optional. Token references are
 * surrounded with curlie braces.
 * 
 * Aggregator options are:
 * <table width="50%">
 ...
 * <s>NOT YET IMPLEMENTED</s> (items with strikeout are not yet supported by LogicCriteria)
 * 
 * For example, the following two criteria would be functionally the same:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * This class is used by the LogicCriteria.<em>parse(String)</em> method and
 * should not be accessed directly.
 * 
 * <h5>Developer notes</h5>
 * <ul>
 * <li>Parentheticals are not yet supported, but could be used to control
 * precedence.</li>
...
public class LogicQueryParser {
	...
	static abstract class Aggregator {
		abstract LogicCriteria apply(LogicCriteria criteria, Matcher match)
	...
	static {
		aggregators.put(regex("EXIST"), new Aggregator() {
			LogicCriteria apply(LogicCriteria criteria, Matcher match)
			        throws LogicQueryParseException {
		...
		aggregators.put(regex("MIN(IMUM)?(\\s+(\\d+) FROM)?"),
		                new Aggregator() {
			                LogicCriteria apply(LogicCriteria criteria,
			                        Matcher match)
			                        throws LogicQueryParseException {
				                throw new LogicQueryParseException("Logic query MINIMUM not implemented");
			                }
		                });
		aggregators.put(regex("MAX(IMUM)?(\\s+(\\d+)\\s+FROM)?"),
		                new Aggregator() {
			                LogicCriteria apply(LogicCriteria criteria,
			                        Matcher match)
			                        throws LogicQueryParseException {
		...
		aggregators.put(regex("(LAST|LATEST)(\\s+(\\d+)\\s+FROM)?"),
		                new Aggregator() {
			                LogicCriteria apply(LogicCriteria criteria,
			                        Matcher match)
			                        throws LogicQueryParseException {
				                if (match.group(3) != null) {
					                ...
					                if (n < 1)
						                throw new LogicQueryParseException("Logic query invalid syntax for '"
						                        + match.group(1)
						                        + " n FROM'. n must be positive whole number: "
						                        + match.group(3));
					                // criteria = criteria.first(n);
					                throw new LogicQueryParseException("Logic query feature not yet supported: "
					                        + match.group(1) + " " + n);
				                }
				                return criteria.last();
			                }
		                });
		aggregators.put(regex("(FIRST|EARLIEST)(\\s+(\\d+)\\s+FROM)?"),
		                new Aggregator() {
			                LogicCriteria apply(LogicCriteria criteria,
			                        Matcher match)
			                        throws LogicQueryParseException {
				                if (match.group(3) != null) {
					                ...
					                if (n < 1)
						                throw new LogicQueryParseException("Logic query invalid syntax for 'FIRST n FROM'. n must be positive whole number: "
						                        + match.group(3));
					                // criteria = criteria.first(n);
					                throw new LogicQueryParseException("Logic query feature not yet supported: FIRST "
					                        + n);
				                }
	...
	static {
		// Accept both < and LT
		conditions.put(regex("(\\>|\\>=|=|==|\\<\\>|!=|\\<|\\<=|GT|GTE|EQ|NE|LT|LTE)\\s+(.*?)\\s*"),
		               new Condition() {
			               LogicCriteria apply(LogicCriteria criteria,
			                       Matcher match)
			                       throws LogicQueryParseException {
				               return applyOperator(criteria,
				                                    match.group(1),
				                                    match.group(2));
			               }
		               });
		conditions.put(regex("WITHIN\\s+(\\d+)\\s+(SECONDS|MINUTES|HOURS|DAYS|WEEKS|MONTHS|YEARS)"),
		               new Condition() {
			               LogicCriteria apply(LogicCriteria criteria,
			                       Matcher match)
			                       throws LogicQueryParseException {
				               return applyWithin(criteria,
				                                  match.group(1),
				                                  match.group(2));
			               }
		               });
		...
		conditions.put(regex("BETWEEN\\s+" + datePattern + "\\s+AND\\s+"
		        + datePattern), new Condition() {
			LogicCriteria apply(LogicCriteria criteria, Matcher match)
			        throws LogicQueryParseException {
				// return
	 ...
	 * @return the equivalent LogicCriteria object for the given logic query
	 *         string
	 * @throws LogicQueryParseException
	 */
	protected static LogicCriteria parse(String query)
	        throws LogicQueryParseException {

		// First, we try to recognize the basic query format
		Matcher queryMatcher = queryPattern.matcher(query);
		if (!queryMatcher.matches())
			throw new LogicQueryParseException("Invalid or unsupported logic query: "
			        + query);
		...
		if (condition != null) {
			...
			if (!conditionMatched)
				...
				throw new LogicQueryParseException("Logic query unrecognized or invalid condition: "
				        + condition);
		}
		...
		if (aggregator != null) {
			...
			if (!aggregatorMatched)
				...
				throw new LogicQueryParseException("Logic query unrecognized or invalid aggregator: "
				        + aggregator);
		}
	...
	/**
	 * Internal method for applying operator conditions to a criteria. Only GT,
	 * LT, GTE, LTE, EQ, and NE are supported
	 */
	private static LogicCriteria applyOperator(LogicCriteria criteria,
	        String op, String operand) throws LogicQueryParseException {
		Double operandValue = 0d;
		try {
			operandValue = Double.parseDouble(operand);
		} catch (Exception e) {
			throw new LogicQueryParseException("Unable to parse value for logic query condition: "
			                                           + operand,
			                                   e);
		}
	 ...
	 */
	private static LogicCriteria applyWithin(LogicCriteria criteria,
	        String duration, String units) throws LogicQueryParseException {
	...
	/**
	 * þ Internal method for parsing date strings into an equivalent Java
	 * object. Accepts "TODAY" for today's date.
	 */
