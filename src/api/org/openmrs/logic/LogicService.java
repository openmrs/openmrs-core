 ...
 * business logic in the form of logic <em>rules</em>. Rules may be run
 * against a single patient or a set of patients.
 * 
 * Rules are registered under a unique string <em>token</em>. Later evaluation and/or
 * retrieval of the rule is done through the token. Tokens can be tagged with
 * any number of string word/phrases to simplify organization and lookup of
 * tokens.
 * 
 * Data source results can be obtained directly by using a token in the form 
 * <em>@foo.bar</em>, where <em>foo</em> is the logic data source name and <em>bar</em>
 * is the key for that data source.  For example, the token <em>@person.gender</em>
 * is a direct reference to the <em>gender</em> key of the <em>person</em> logic data
 * source.
 * 
 * <h3>Example Usage</h3>
 * <pre>
...
public interface LogicService {
	 ...
	 * @param tags words or phrases associated with this token (do not need to
	 *        be unique)
	 * @param rule new rule to be registered
	 * @throws LogicException
	 */
	 ...
	 * 
	 * @param token lookup key ("token") under which rule to be removed is
	 *        registered
	 * @throws LogicException
	 */
	 ...
	 * @param criteria question to be answered (along with the token) for the
	 *        given patient
	 ...
	 * 
	 * @param who patients for whom the query is to be run
	 * @param criteria question to be answered (along with the token) for each
	 *        patient
	 * @return result for each patient
	 * @throws LogicException
	 */
	...
	/**
	 * 
	 * Adds a data source to the logic service. Data sources provide access to
	 * granular data that can be combined by rules to derive higher level
	 * information.
	 * 
	 ...
	 * @throws LogicException
	 */
	public void registerLogicDataSource(String name, LogicDataSource logicDataSource)
	        throws LogicException;
	...
	/**
	 * Adds the given logic data sources to the list of current
	 * data sources on this logic service
	 * 
	 * @param logicDataSources
	 */
	public void setLogicDataSources(
	        Map<String, LogicDataSource> logicDataSources) throws LogicException;
	 ...
	 * @return the logic data source with the given name or <code>null</code>
