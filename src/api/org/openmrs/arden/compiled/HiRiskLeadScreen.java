...
private ArdenValue Last_Pb(){
	...
	return dataSource.eval(patient, ardenClause.concept(concept).last(2).within().past().Days(330));
}

private ArdenValue Qual_Pb(){
	Concept concept;
	concept = Context.getConceptService().getConceptByName("Qualitative_Blood_Lead");
	return dataSource.eval(patient, ardenClause.concept(concept).last(1).within().past().Days(330));
}

private ArdenValue HousePre50(){
	Concept concept;
	concept = Context.getConceptService().getConceptByName("HouseBltPre1950");
	return dataSource.eval(patient, ardenClause.concept(concept).last(1).within().past().Years(2));
}

private ArdenValue RenovatedPre78(){
	Concept concept;
	concept = Context.getConceptService().getConceptByName("RenovatedPre78");
	return dataSource.eval(patient, ardenClause.concept(concept).last(1).within().past().Days(365));
}
