...
public class OrderServiceTest extends TestCase {
	...
	public void testOrderCreateUpdateDelete() throws Exception {
		...
		User orderer1 = us.getUserByUsername("admin");
		...
		User orderer2 = us.getUserByUsername("admin");
		...
		if (order3.getConcept() != null && order1.getConcept() != null)
			assertFalse(order3.getConcept().equals(order1.getConcept()));
		assertFalse(order3.getOrderType().equals(order1.getOrderType()));
		//assertFalse(order3.getOrderer().equals(order1.getOrderer()));
		assertFalse(order3.getEncounter().equals(order1.getEncounter()));
		assertFalse(order3.getInstructions().equals(order1.getInstructions()));
		assertFalse(order3.getStartDate().equals(order1.getStartDate()));
		assertFalse(order3.getAutoExpireDate().equals(order1.getAutoExpireDate()));
		
		orderService.voidOrder(order1, "testing void function");
		
		Order order4 = orderService.getOrder(order1.getOrderId());
		
		assertFalse(order4.getVoidReason().equals(order3.getVoidReason()));
		assertFalse(order4.getVoidedBy().equals(order3.getVoidedBy()));
		...
		assertFalse(order5.getDiscontinuedReason().equals(order1.getDiscontinuedReason()));
		//System.out.println("order5.getDiscontinuedBy: " + order5.getDiscontinuedBy().getUsername());
		assertFalse(order5.getDiscontinuedBy().equals(order1.getDiscontinuedBy()));
	...
	public void testDrugOrderCreateUpdateDelete() throws Exception {
		...
		Date start1 = new Date();
		Date autoexpire1 = new Date();
		Drug drugInventoryId1 = null; //conceptService.getDrugs().get(1);
		...
		Date start2 = new Date();
		Date autoexpire2 = new Date();
		Drug drugInventoryId2 = null; //conceptService.getDrugs().get(2);
		...
		if (drugorder3.getConcept() != null && drugorder1.getConcept() != null)
			assertFalse(drugorder3.getConcept().equals(drugorder1.getConcept()));
		assertFalse(drugorder3.getOrderType().equals(drugorder1.getOrderType()));
		//assertFalse(drugorder3.getOrderer().equals(drugorder1.getOrderer()));
		assertFalse(drugorder3.getEncounter().equals(drugorder1.getEncounter()));
		assertFalse(drugorder3.getInstructions().equals(drugorder1.getInstructions()));
		assertFalse(drugorder3.getStartDate().equals(drugorder1.getStartDate()));
		assertFalse(drugorder3.getAutoExpireDate().equals(drugorder1.getAutoExpireDate()));
		if (drugorder3.getDrug() != null && drugorder1.getDrug() != null)
			assertFalse(drugorder3.getDrug().equals(drugorder1.getDrug()));
		assertFalse(drugorder3.getDose().equals(drugorder1.getDose()));
		assertFalse(drugorder3.getUnits().equals(drugorder1.getUnits()));
		assertFalse(drugorder3.getFrequency().equals(drugorder1.getFrequency()));
		assertFalse(drugorder3.isPrn().equals(drugorder1.isPrn()));
		assertFalse(drugorder3.isComplex().equals(drugorder1.isComplex()));
		assertFalse(drugorder3.getQuantity().equals(drugorder1.getQuantity()));
		...
		assertFalse(drugorder4.getVoidReason().equals(drugorder3.getVoidReason()));
		assertFalse(drugorder4.getVoidedBy().equals(drugorder3.getVoidedBy()));
		...
		assertFalse(drugorder5.getDiscontinuedReason().equals(drugorder1.getDiscontinuedReason()));
		//System.out.println("drugorder5.getDiscontinuedBy: " + drugorder5.getDiscontinuedBy().getUsername());
		assertFalse(drugorder5.getDiscontinuedBy().equals(drugorder1.getDiscontinuedBy()));
