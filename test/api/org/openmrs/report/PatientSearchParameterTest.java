...
public class PatientSearchParameterTest extends BaseContextSensitiveTest {
	 ...
	 * determined in an EvaluationContext 1. Create PatientSearch, add SearchArguments as
	 * Parameters, and persist in the database. 2. Create an EvaluationContext to assign values to
	 * the PatientSearch Parameters 3. Retrieve the PatientSearch from the database and test if its
	 * Parameters evaluate to the EvaluationContext Parameter values.
	 * 
	 * @throws Exception
	 */
	@Test
	public void shouldPatientSearchParameter() throws Exception {
		...
		context.addParameterValue(new Parameter("test.salutation", "TestSalutation", String.class, "WrongSalutation"),
		    salutation);
		...
		for (Parameter p : parameters) {
			if ("TestName".equals(p.getLabel())) {
				...
				assertEquals(df.format(today), df.format((Date) context
				        .evaluateExpression(ps2.getArgumentValue(p.getName()))));
				log.info("Testing Parameters: " + context.evaluateExpression(p.getName()).toString() + " "
				        + df.format(today));
			} else {
