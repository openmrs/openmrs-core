...
public class LogicCriteriaTest {
	...
	public void shouldLogicCriteria() {
		LogicCriteria criteria = new LogicCriteria("CD4 COUNT").within(Duration.months(6)).exists();
		...
		criteria = new LogicCriteria("PROBLEM ADDED").contains("HUMAN IMMUNODEFICIENCY VIRUS").first();
		assertEquals("PROBLEM ADDED", criteria.getRootToken());
		
		criteria = new LogicCriteria("PROBLEM ADDED").contains("HIV INFECTED").first();
		assertEquals("PROBLEM ADDED", criteria.getRootToken());
		
		criteria = new LogicCriteria("PROBLEM ADDED").contains("ASYMPTOMATIC HIV INFECTION").first();
		assertEquals("PROBLEM ADDED", criteria.getRootToken());
		...
		criteria = new LogicCriteria("CD4 COUNT").last().lt(350).and(new LogicCriteria("%%orders.ACTIVE MEDS").notExists());
		assertEquals("CD4 COUNT", criteria.getRootToken());
