...
/**
 * Filter that compresses output with gzip (assuming that browser supports gzip). Code from
 * <a href="http://www.onjava.com/pub/a/onjava/2003/11/19/filters.html">
	...
	private boolean isCompressedRequestForPathAccepted(String path) {
		try {
			if (cachedGZipCompressedRequestForPathAccepted == null) {
				cachedGZipCompressedRequestForPathAccepted = Context.getAdministrationService()
				        .getGlobalProperty(OpenmrsConstants.GLOBAL_PROPERTY_GZIP_ACCEPT_COMPRESSED_REQUESTS_FOR_PATHS, "");
			}
