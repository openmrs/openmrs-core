...
public class HIVPositiveRule implements Rule {
    ...
    public Result eval(LogicContext context, Patient patient,
        ...
        try {
            ageOK = context.read(patient, context.getLogicDataSource("obs"),
                    new LogicCriteria("AGE").gt(1)).toBoolean();
            if (!ageOK)
                return Result.emptyResult();

            // we find the first HIV diagnosis
            allDiagnoses.add(Context.getLogicService().eval(
                    patient,
                    new LogicCriteria("PROBLEM ADDED").contains(
            ...
            allDiagnoses.add(Context.getLogicService().eval(patient,
                    new LogicCriteria("HIV VIRAL LOAD").first()));
            ...
            // first CD4 COUNT < 200
            allDiagnoses.add(Context.getLogicService().eval(patient,
                    new LogicCriteria("CD4 COUNT").lt(200).first()));
    ...
    public String[] getDependencies() {
        return new String[] { "HUMAN IMMUNODEFICIENCY VIRUS", "HIV INFECTED",
                "ASYMPTOMATIC HIV INFECTION", "HIV VIRAL LOAD",
                "HIV VIRAL LOAD, QUALITATIVE", "CD4 COUNT" };
    }
